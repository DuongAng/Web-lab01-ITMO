...
function redrawGraph(r) {
    ...
    //1/4 hinh tron tim hieu sau
    ctx.arc(w / 2, h / 2, hatchGap, Math.PI*2,  Math.PI/2 , false);
    //hinh cn
    ctx.lineTo(w / 2, h / 2);
    ctx.lineTo(w / 2, h / 2 - hatchGap * 2);
...
function printDotOnGraph(xCenter, yCenter, isHit) {
    redrawGraph(rValue);
    ctx.fillStyle = isHit ? '#1f428f' : '#ff0000'
